...
**Multiple Variations:**
- Generates **3 caption variations** per run with different angles
- User picks their favorite or regenerates
- Variations: one storytelling, one technical, one concise
...
- Variations count: 1-5 (default: 3)
...
# Generate multiple variations (default: 3)
postgen generate ./my-project --variations=5
...
interface ProjectContext {
  ...
  // Monorepo info
  isMonorepo: boolean;
  monorepoPackages?: { name: string; path: string; description?: string }[];
  scannedPackage?: string;     // If scanning specific package in monorepo
...
interface GenerationOptions {
  ...
  variations: number;          // Number of caption variations (default: 3, max: 5)
  includeTemplateCard: boolean;
  includeAiImage: boolean;
  templateName?: string;
  monorepoPackage?: string;    // Specific package to scan in monorepo
}
```
...
> [!NOTE]
> Web dashboard is **local-only** (`postgen serve`). No cloud deployment needed. Removed Vercel deploy workflow.
...
| # | Question | Resolution |
|---|----------|------------|
| Q1 | Project name | **`postgen`** — confirmed final |
| Q2 | Web dashboard deployment | **Local-only** — `postgen serve` on localhost, no cloud hosting |
| Q3 | Monorepo scanning | **Yes** — support `--package=apps/web` flag for monorepo scanning |
| Q4 | Multiple caption variations | **Yes** — generate 3 variations by default, configurable up to 5 |
